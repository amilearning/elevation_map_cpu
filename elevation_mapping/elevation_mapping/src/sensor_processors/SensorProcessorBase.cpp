...
bool SensorProcessorBase::readParameters() {
  ...
  nodeHandle_.param("/elevation_mapping/sensor_processor/ignore_points_above", parameters.ignorePointsUpperThreshold_,
                    std::numeric_limits<double>::infinity());
  nodeHandle_.param("/elevation_mapping/sensor_processor/ignore_points_below", parameters.ignorePointsLowerThreshold_,
                    -std::numeric_limits<double>::infinity());
  nodeHandle_.param("/elevation_mapping/sensor_processor/x_ignore_points_below", parameters.ignorePointsLowerThreshold_in_x_,
                  -std::numeric_limits<double>::infinity());
...
void SensorProcessorBase::removePointsOutsideLimits(PointCloudType::ConstPtr reference, std::vector<PointCloudType::Ptr>& pointClouds) {
  ...
  if (!std::isfinite(parameters.ignorePointsLowerThreshold_) && !std::isfinite(parameters.ignorePointsUpperThreshold_)) {
    ROS_DEBUG("removePointsOutsideLimits returned");
    return;
  }
...
bool SensorProcessorBase::filterPointCloud(const PointCloudType::Ptr pointCloud) {
  ...
  passThroughFilter.setFilterFieldName("x");  // TODO(max): Should this be configurable?  
  passThroughFilter.setFilterLimits(parameters.ignorePointsLowerThreshold_in_x_, 1000.0);
  pcl::IndicesPtr insideIndeces(new std::vector<int>);
  passThroughFilter.filter(*insideIndeces);
  ...
  // for (auto& pointCloud : pointClouds) {
    ...
    extractIndicesFilter.setInputCloud(pointCloud);
    extractIndicesFilter.setIndices(insideIndeces);
    PointCloudType tempPointCloud2;
    extractIndicesFilter.filter(tempPointCloud2);
    pointCloud->swap(tempPointCloud2);
  // }
