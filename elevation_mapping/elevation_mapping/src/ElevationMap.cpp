...
void ElevationMap::setRobotPoseCache(message_filters::Cache<geometry_msgs::PoseWithCovarianceStamped>* robotPoseCache) {
    robotPoseCache_ = robotPoseCache;
}

void ElevationMap::setRobotPoseCacheSize(int size) {
    if (robotPoseCache_) {
        robotPoseCache_->setCacheSize(size);
    }
}
...
bool ElevationMap::fuse(const grid_map::Index& topLeftIndex, const grid_map::Index& size) {
  ...
  //  // do not fuse near robot footprint
  //   auto latest_time = robotPoseCache_->getLatestTime();
  //   boost::shared_ptr<geometry_msgs::PoseWithCovarianceStamped const> poseMessage = robotPoseCache_->getElemBeforeTime(latest_time);    
  //   if (poseMessage) {     
  //    grid_map::Position cur_robot_xy(poseMessage->pose.pose.position.x, poseMessage->pose.pose.position.y);
  //    double distanceToRobot = (requestedSubmapPosition-cur_robot_xy).norm();
  //     if (distanceToRobot < parameters.non_fov_distance_){     
  //       // if(fusedMap_.exists("variance")){
  //       //   rawMapCopy.at("elevation", *areaIterator) = fusedMap_.at("elevation", *areaIterator);
  //       //   rawMapCopy.at("lower_bound", *areaIterator) = fusedMap_.at("elevation", *areaIterator) - 2.0 * sqrt(fusedMap_.at("variance", *areaIterator));
  //       //   rawMapCopy.at("upper_bound", *areaIterator) =fusedMap_.at("elevation", *areaIterator) + 2.0 * sqrt(fusedMap_.at("variance", *areaIterator));
  //       //   rawMapCopy.at("color", *areaIterator) = fusedMap_.at("color", *areaIterator);
  //       // }        
  //       continue;
  //     } 
  //   }
    ...
    // ROS_INFO("robot pose in x = %f, and y = %f",pose_.getPosition().x(), pose_.getPosition().y() );
    // double distanceToRobot = (requestedSubmapPosition-cur_robot_pose_).norm();
    // if (distanceToRobot < parameters.non_fov_distance_){
    //   ROS_INFO("NO Fuse distance =  %f", distanceToRobot);
    //   continue;
    // } 
